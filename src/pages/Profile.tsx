...
import { Flame, Trophy, BookOpen, Star, Award } from "lucide-react";
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Day Streak", value: userProfile.streak, icon: Flame, color: "text-warning" },
            ...
            { label: "Skills", value: userProfile.skills.length, icon: Star, color: "text-accent" },
          ].map((stat) => (
      ...
      <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {userProfile.badges.map((badge) => (
            <Card key={badge.id} className={`shadow-card border-border ${!badge.earned ? "opacity-40" : ""}`}>
              <CardContent className="p-4 text-center">
